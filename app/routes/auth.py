...
def signup():
    ...
    try:
        ...
        if not validate_phone(phone):
            return jsonify({'error': 'Invalid phone format. Use +966XXXXXXXXX or 05XXXXXXXX'}), 400
...
def login():
    ...
    try:
        ...
        if doctor.status == 'rejected':
            return jsonify({'error': 'Your registration has been rejected'}), 403
...
# ── ADMIN LOGIN ───────────────────────────────────────────────
@auth_bp.route('/api/admin/login', methods=['POST', 'OPTIONS'])
def admin_login():
    if request.method == 'OPTIONS':
        return jsonify({}), 200
    try:
